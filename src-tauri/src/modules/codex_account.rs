...
/// Legacy data directory from early private builds.
...
/// Return the local Codex account details directory.
...
"#,
    ...
    #[test]
    fn validate_api_key_credentials_appends_v1_for_root_base_url() {
        let (_, api_base_url) =
            validate_api_key_credentials("sk-test-key", Some("https://relay.local/"))
                .expect("root base url should be normalized to /v1");
        assert_eq!(api_base_url.as_deref(), Some("https://relay.local/v1"));
    }
